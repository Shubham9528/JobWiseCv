...
export default function InputBox({ setInputBox }) {
  ...
  const handleSubmit = async (event) => {
    ...
    if (inputValue.length > 20) {
      ...
      try {
        const response = await axios.post(`${process.env.REACT_APP_BACKEND_PORT}/process`, {
          data: inputValue,
        });
    ...
    <div className="flex flex-col items-center p-8 bg-gradient-to-b from-indigo-100 to-blue-50 min-h-screen font-sans">
      ...
      >
        ...
        <p className="mt-8 text-lg text-gray-700">
         Status: <span className="font-semibold text-lg text-green-600">{process.env.REACT_APP_BACKEND_PORT || ("error--" + status)}</span>
        </p>
        <button
          type="submit"
